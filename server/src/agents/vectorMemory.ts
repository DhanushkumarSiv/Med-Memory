export interface VectorRagRecord {
  resourceType: string;
  resourceId: string;
  recordedAt: string;
  sourceName: string;
...
interface QueryIntent {
  wantsCondition: boolean;
  wantsMedication: boolean;
  wantsLabOrResult: boolean;
  wantsAllergy: boolean;
  wantsCardiac: boolean;
}
...
function summarizeRecord(record: VectorRagRecord): string {
  ...
  if (resourceType === "Observation") {
    const observationName = safeText((payload.code as Record<string, unknown> | undefined)?.text, "Observation");
...
function recordToDocument(record: VectorRagRecord): string {
  const payload = record.payload;
...
function inferIntent(tokens: Set<string>): QueryIntent {
  return {
    ...
    wantsLabOrResult:
      tokens.has("lab") || tokens.has("result") || tokens.has("scan") || tokens.has("observation") || tokens.has("report"),
    wantsAllergy: tokens.has("allergy") || tokens.has("allergic") || tokens.has("reaction"),
    wantsCardiac:
      tokens.has("cardiac") || tokens.has("heart") || tokens.has("cardio") || tokens.has("ecg") || tokens.has("echo"),
  };
}

function computeIntentBoost(intent: QueryIntent, record: VectorRagRecord, content: string): number {
  let boost = 0;
...
function computeSignature(records: VectorRagRecord[]): string {
  return records
...
export class InMemoryVectorMemory {
  private readonly patientIndexes = new Map<string, PatientIndex>();

  private buildIndex(records: VectorRagRecord[]): IndexedRecord[] {
  ...
  private ensurePatientIndex(patientId: string, records: VectorRagRecord[]): PatientIndex {
  ...
  search(patientId: string, records: VectorRagRecord[], query: string, limit = 6): VectorMatch[] {
