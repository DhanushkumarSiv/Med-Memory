...
let demoRepairInFlight: Promise<void> | null = null;
...
function buildToken(payload: Record<string, unknown>): string {
  ...
  return jwt.sign(payload, process.env.JWT_SECRET ?? "medmemory-dev-secret", {
    ...options,
  });
}
...
async function ensureDemoSeedData(): Promise<void> {
  if (!demoRepairInFlight) {
    demoRepairInFlight = seedGraph().finally(() => {
...
router.post("/patient/login", async (req: Request, res: Response) => {
  try {
    ...
    let rows = await runQuery<{ p: Record<string, unknown> }>(
      `
    ...
    if (
      ...
      rows = await runQuery<{ p: Record<string, unknown> }>(
        `
        MATCH (p:Patient {abhaId: $abhaId})
        RETURN p
        LIMIT 1
...
router.post("/provider/login", async (req: Request, res: Response) => {
  try {
    ...
    let rows = await runQuery<{ pr: Record<string, unknown> }>(
      `
      MATCH (pr:Provider)
    ...
    if (
      ...
      await ensureDemoSeedData();
      rows = await runQuery<{ pr: Record<string, unknown> }>(
        `
        MATCH (pr:Provider)
        WHERE toLower(pr.loginId) = $loginId
        RETURN pr
...
router.post("/provider/lookup-patient", async (req: Request, res: Response) => {
  try {
    ...
    const { abhaId } = req.body as { abhaId?: string };
    if (!abhaId) {
...
router.post("/provider/request-otp", async (req: Request, res: Response) => {
  try {
    const token = parseBearer(req);
    if (!token) {
      return res.status(401).json({ message: "Missing authorization token" });
...
router.post("/provider/verify-otp", async (req: Request, res: Response) => {
  try {
    ...
    if (!consentToken) {
      ...
      if (scopeString) {
        scopes = JSON.parse(scopeString) as string[];
      }
    }
...
router.get("/dev/last-otp", (_req: Request, res: Response) => {
  ...
  return res.json({ otp: getLastDevOtp() });
});
