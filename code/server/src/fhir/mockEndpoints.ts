export interface MockFhirSource {
  id: string;
  sourceName: string;
  sourceType: "hospital" | "lab" | "pharmacy" | "wearable";
  fhirBaseUrl: string;
}

export interface MockFhirResource {
  resourceType:
    | "Patient"
    | "Condition"
    | "MedicationRequest"
...
export interface MockPatientRecord {
  ...
  phone: string;
  city: string;
  sources: MockFhirSource[];
  resources: MockFhirResource[];
}

const priyaSources: MockFhirSource[] = [
  {
    id: "apollo-chennai",
...
const rajanSources: MockFhirSource[] = [
  {
    id: "kokilaben-mumbai",
...
const priyaResources: MockFhirResource[] = [
  {
...
const rajanResources: MockFhirResource[] = [
  {
    resourceType: "Patient",
...
export const mockPatientRecords: MockPatientRecord[] = [
  {
...
export function getFhirPatientByAbhaId(abhaId: string): Record<string, unknown> | null {
  const match = mockPatientRecords.find((record) => record.abhaId === abhaId);
  if (!match) {
    return null;
  }
...
export function getFhirBundleByAbhaId(abhaId: string): Record<string, unknown> | null {
  const match = mockPatientRecords.find((record) => record.abhaId === abhaId);
  if (!match) {
    return null;
  }
