...
function extractCharacterTrigrams(text) {
...
function scoreDomainGroups(text) {
...
function resizeVector(values, dimension) {
...
function embedLocally(text, dimension) {
...
function averageVectors(values) {
    if (values.length === 0) {
        return [];
    }
...
function flattenUnknownEmbedding(payload) {
    if (!Array.isArray(payload) || payload.length === 0) {
        return null;
    }
    if (typeof payload[0] === "number") {
        return payload.filter((value) => Number.isFinite(value));
    }
    if (Array.isArray(payload[0])) {
        const rows = payload
            .filter((item) => Array.isArray(item))
            .map((row) => row.filter((value) => typeof value === "number" && Number.isFinite(value)));
        if (rows.length === 0) {
            return null;
        }
...
async function embedWithHuggingFace(text, dimension) {
...
async function embedText(text, dimension = DEFAULT_EMBEDDING_DIMENSION) {
